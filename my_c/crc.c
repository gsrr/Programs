...
unsigned int crc32a_test(unsigned char *message) {
   int i, j;
   ...
   while (message[i] != 0) {
      ...
      {    // Do eight times.
         //if ((int)(crc ^ byte) < 0)
                printf("Round:%d, %x:%x, ret:%d\n", j, crc, byte, byte >> 24 & 0x80);      
         unsigned int tmp = crc ^ byte;
         if ((tmp >> 24 & 0x80)  != 0)
         {
...
unsigned int crc32a(unsigned char *message) {
   ...
   i = 0;
   // crc的initial value 為 0xffffffff, 也有的演算法是為0,當然後續算法有些東西就要反過來.
   crc = 0xFFFFFFFF;    
   while (message[i] != 0) {
      byte = message[i];            // Get next byte.
      byte = reverse(byte);         // 32-bit reversal, why reverse?
      for (j = 0; j <= 7; j++) 
      {    // Do eight times.
                printf("Round:%d, %x:%x, ret:%d\n", j, crc, byte, crc ^ byte);      
         if ((int)(crc ^ byte) < 0) // 好像就是把byte加到crc上面去, 加上去後如果是第一個bit是1的狀況.
              crc = (crc << 1) ^ 0x04C11DB7;
         else crc = crc << 1;
         byte = byte << 1;          // Ready next msg bit.
      }
